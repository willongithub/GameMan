...
DESCRIPTION:
	Main Entry for GameMan Console
*************************************************************************/
...
{
	byte i, column, row, page;
	column = 40;
	select_column(column);
	for (i = 0; i < MAX_PAGES; i++)
	{
		select_page(i);
		pixel = (ON | (buffer[column][i]));	
